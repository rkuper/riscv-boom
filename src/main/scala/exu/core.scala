  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  val mem_iss_unit     = if (enableCountingIssue) Module(new IssueUnitAgeMatrix(memIssueParam, numIntIssueWakeupPorts))
  // val mem_iss_unit     = if (enableCountingIssue) Module(new IssueUnitIndexCollapsing(memIssueParam, numIntIssueWakeupPorts))
  // val mem_iss_unit     = if (enableCountingIssue) Module(new IssueUnitCounting(memIssueParam, numIntIssueWakeupPorts))
  // val mem_iss_unit     = if (enableCountingIssue) Module(new IssueUnitCollapsing(memIssueParam, numIntIssueWakeupPorts))
  ...
  val int_iss_unit     = if (enableCountingIssue) Module(new IssueUnitAgeMatrix(intIssueParam, numIntIssueWakeupPorts))
  // val int_iss_unit     = if (enableCountingIssue) Module(new IssueUnitIndexCollapsing(intIssueParam, numIntIssueWakeupPorts))
  // val int_iss_unit     = if (enableCountingIssue) Module(new IssueUnitCounting(intIssueParam, numIntIssueWakeupPorts))
  // val int_iss_unit     = if (enableCountingIssue) Module(new IssueUnitCollapsing(intIssueParam, numIntIssueWakeupPorts))

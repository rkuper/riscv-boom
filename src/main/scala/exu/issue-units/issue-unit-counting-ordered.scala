  ...
  extends IssueUnit(params.numEntries, params.issueWidth, numWakeupPorts, params.iqType, params.dispatchWidth)
{
  ...
  //-------------------------------------------------------------
  // Issue Slot Age Records

	val ageCounterSize = numIssueSlots * 1
  val issue_records = RegInit(VecInit(Seq.fill(numIssueSlots) {
    val issue_record = Wire(new IssueRecord(log2Ceil(ageCounterSize)))
    issue_record.age      := 0.U
    issue_record.valid    := false.B
    issue_record.overflow := false.B
    issue_record
  }))

  val age_counter = Reg(UInt(log2Ceil(ageCounterSize).W))
  val something_available = num_available != 0
  val will_be_available_indexes = will_be_available.zipWithIndex.map { case (bool, index) => bool -> index.U(numIssueSlots.W) }


  // When something can be queued...
  when (something_available.asBool) {
    // When the counter needs to be reset, set to 0 and set the overflow bit in every valid record
    when (age_counter === (ageCounterSize-1).U) {
      age_counter := 0.U
      for (i <- 0 until numIssueSlots) {
        when (issue_records(i).valid) {
          issue_records(i).overflow := true.B
        }
      }
    } .otherwise { // Otherwise, just increment the counter
      age_counter := age_counter + 1.U
    }
  }

	for (i <- 0 until numIssueSlots) {
		when (will_be_available(i)) {
    	issue_records(i).valid := true.B
    	issue_records(i).age := age_counter
		}
	}
  ...
  for (i <- 0 until numIssueSlots) {
    ...
    for (w <- 0 until issueWidth) {
      ...
      when (requests(i) && !uop_issued && can_allocate && !port_issued(w)) {
				issue_records(i).valid    := false.B
				issue_records(i).overflow := false.B
...
class IssueRecord(val ageWidth: Int) extends Bundle {

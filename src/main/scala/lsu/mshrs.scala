  ...
  with HasL1HellaCacheParameters
{
  ...
  // val prefetcher: DataPrefetcher = if (enablePrefetching) Module(new OffsetPrefetcher)
  val prefetcher: DataPrefetcher = if (enablePrefetching) Module(new StridePrefetcher)
